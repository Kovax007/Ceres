...
public sealed class MultiGPUEnginePool : IDisposable
{
                             ...
                             int[] deviceIds, int minBatchSizePerGPU, string cacheDir)
  {
    ...
    // Pre-build unified engine with timing cache for Exact mode multi-GPU ONNX builds
    ...
    bool isOnnx = ext != ".engine" && ext != ".plan";
    if (isOnnx && mode == EnginePoolMode.Exact && deviceIds.Length > 1 && !hasDuplicateDevices)
    {
      int[] allSizes = sizesPerGPU[0]; // All GPUs use the same batch sizes
      int[] sortedSizes = (int[])allSizes.Clone();
