...
public class ManagerChooseBestMoveMCGS
{
  ...
  public BestMoveInfoMCGS BestMoveCalc
  {
    get
    {
      MGPosition position = Node.CalcPosition();
...
//       && Manager.ParamsSearch.TestFlag
  ...
  private BestMoveInfoMCGS DoCalcBestMove()
  {
    ...
    if (Manager.ParamsSearch.BestMoveMode == ParamsSearch.BestMoveModeEnum.RegularizedPolicyOptimizationLow)
    {
      ...
      GEdge bestChildEdge = RPOTests.BestMove(Node, float.NaN, Node.NumEdgesExpanded, LAMBDA, LAMBDA_POWER);
  ...
  ///   - Root Q is greater than 0.3 (winning position)
  ...
  private BestMoveInfoMCGS TryOverrideWithMoreIrreversibleMove(MGPosition position,
                                                               ...
                                                               GEdge[] edgesSortedQ)
  {
    if (!IsFinalBestMoveCalc || !Manager.ParamsSearch.TestFlag || true) // DISABLED 
    {
      return currentBest;
    }

    const float MIN_ROOT_Q_FOR_OVERRIDE = 0.15f;
    const float MAX_Q_DIFFERENCE = 0.10f;

    const int MIN_PLY_ORIGINAL_NOT_IRREVERSIBLE = 12;
    const float MIN_SHORTER_IRREVERISLBE_DIVISOR = 1.5f;
    ...
    if (moveA.IsNull || moveA.ChildNode.IsNull)
    {
      return currentBest;
    }

    int? plyA = moveA.ChildNode.PlyUntilPVIsIrreversibleMove();
    ...
    foreach (GEdge candidateB in edgesSortedQ)
    {
      ...
      // Skip if child node doesn't exist
      if (candidateB.IsNull || candidateB.ChildNode.IsNull)
      {
        continue;
      }

      // Skip if the candidate move is itself irreversible (pawn move or capture)
      // since there's no benefit in overriding to reach irreversibility sooner
      // if the override move itself is already irreversible
      ...
      if (qDifference < 0 || qDifference > MAX_Q_DIFFERENCE)
      {
        continue;
      }
      ...
      int? plyB = candidateB.ChildNode.PlyUntilPVIsIrreversibleMove();
      ...
      else if (plyA.HasValue && plyB.HasValue && plyB.Value < plyA.Value / MIN_SHORTER_IRREVERISLBE_DIVISOR)
      {
      ...
      if (shouldOverride)
      {
        ...
        if (DUMP)
        {
          string plyAStr = plyA.HasValue ? plyA.Value.ToString() : "-";
          string plyBStr = plyB.HasValue ? plyB.Value.ToString() : "-";

          Console.WriteLine();
          Console.WriteLine("=== IRREVERSIBLE MOVE OVERRIDE ===");
          Console.WriteLine($"  Original move: {moveA.MoveMG} Q={-moveA.Q:F4} PlyToIrreversible={plyAStr}");
          Console.WriteLine($"  Override move: {candidateB.MoveMG} Q={-candidateB.Q:F4} PlyToIrreversible={plyBStr}");
          Console.WriteLine($"  Root Q: {Node.Q:F4}  Q difference: {qDifference:F4}");

          // Create temporary BestMoveInfoMCGS for DumpFullInfo
          BestMoveInfoMCGS overriddenBestMove = new BestMoveInfoMCGS(BestMoveInfoMCGS.BestMoveReason.SearchResult,
                                                                      position,
                                                                      candidateB,
          ...
          Manager.DumpFullInfo(overriddenBestMove, Node, default, Console.Out, "Irreversible move override");
        }
