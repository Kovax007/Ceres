...
public partial class GParentsStore
{
  ...
  public GParentsStore(GraphStore parentStore, long maxParents, bool coalescedMode, bool tryEnableLargePages)
  {
    ...
    long parentsMultiplier = coalescedMode ? 8 : 1; // In coalesced mode many more nodes will converge on same node
    long setNumDetailSegments = (parentsMultiplier * maxParents) / (GParentDetailsStruct.MAX_ENTRIES_PER_SEGMENT - 1);
  ...
  /// </summary>
  public MemoryBufferOS<GParentDetailsStruct> MemoryBufferOSStore => DetailSegments.MemoryBufferOSStore;
  ...
  private void innerGetParentsForChild(NodeIndex childIndex, Span<int> parents)
  {
    ...
    {
      GParentDetailsStruct segment = DetailSegments.SegmentRef(headerPointer.AsSegmentLinkIndex);
  ...
  public void DumpForEntry(NodeIndex index)
  {
    ...
    {
      ref GParentDetailsStruct segment = ref DetailSegments.SegmentRef(header.AsSegmentLinkIndex);
  ...
  {
    Debug.Assert(nodes[childIndex.index].LockRef.IsLocked);
    ...
    else if (parentsHeaderPointer.IsDirectEntry)
    {
      ...
      ref GParentDetailsStruct newSegment = ref DetailSegments.SegmentRef(segmentIndex);
    ...
    else // if (curValue.IsLink)
    {
      ...
      ref GParentDetailsStruct segment = ref DetailSegments.SegmentRef(parentsHeaderPointer.AsSegmentLinkIndex);
      while (true)
      {
        if (segment.IsFull)
        {
          ...
          {
            segment = ref DetailSegments.SegmentRef(segment.Entries[GParentDetailsStruct.MAX_ENTRIES_PER_SEGMENT - 1].AsSegmentLinkIndex);
          }
          else
          {
            int segmentIndex = DetailSegments.AllocateSegment();
            ...
            newSegment.Entries[0] = segment.Entries[GParentDetailsStruct.MAX_ENTRIES_PER_SEGMENT - 1].AsDirectParentNodeIndex.Index;

            // Update the old segment to point to the new segment (via the last entry)
            segment.Entries[GParentDetailsStruct.MAX_ENTRIES_PER_SEGMENT - 1].SetToSegmentLink(segmentIndex);
