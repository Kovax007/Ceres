...
public static class UCIInfoMCGS
{
                                     ...
                                     bool isChess960 = false)
  {
    ...
    if (showWDL)
    {
      ...
      strWDL = $" wdl {Math.Round((isRoot ? bestMoveEdge.ParentNode.WinP : bestMoveEdge.ParentNode.W) * 1000)} "
             + $"{Math.Round((isRoot ? bestMoveEdge.ParentNode.DrawP : bestMoveEdge.ParentNode.D) * 1000)} "
             + $"{Math.Round((isRoot ? bestMoveEdge.ParentNode.LossP : bestMoveEdge.ParentNode.L) * 1000)}";
    }
