...
{
  /// <summary>
  /// Selection terminator that consults a PositionEvalCache to 
  /// try to return an already evaluated and stored result.
  /// </summary>
  public sealed class SelectTerminatorCache : LeafEvaluatorBaseX
  {
    ...
    public SelectTerminatorCache(PositionEvalCache cache)
    {
      Cache = cache;
    }
    ...
    /// <returns></returns>
    protected override LeafEvaluationResultX DoTryEvaluate(MCTSNode node)
    {
      ...
      if (inCache)
      {
        ...
        LeafEvaluationResultX result = new (cacheEntry.TerminalStatus, cacheEntry.WinP, cacheEntry.LossP, cacheEntry.M, cacheEntry.UncertaintyV);
