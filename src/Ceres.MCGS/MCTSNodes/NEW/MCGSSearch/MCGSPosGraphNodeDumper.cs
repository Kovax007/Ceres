...
namespace Ceres.MCGS.MTGSNodes.Analysis;
...
#if NOT
    string fracVisitStr = "   ";
    if (Depth == 1 && manager?.RootMoveTracker.RunningFractionVisits != null)
    {
      float runningAvg = manager.RootMoveTracker.RunningFractionVisits[IndexInParentsChildren];
      fracVisitStr = $"{Math.Round(runningAvg * 100, 0),3:F0}";
    }

    string recentQAvgStr = "     ";
    if (Depth == 1 && manager?.RootMoveTracker.RunningVValues != null)
    {
      float runningQ = manager.RootMoveTracker.RunningVValues[IndexInParentsChildren];
...
//    writer.Write(" Dep  T #M    FirstVisit   MvWh  MvBl              N  Visits    Policy    Act       V        Q      WPos  DPos  LPos   WTree DTree LTree  VUnc PUnc PIrr VErr");
  ...
  public static HashSet<int> NodesToRootSet(GNode node)
  {
    ...
    while (!node.IsNull)
    {
      ...
      if (node.TryGetSingleParentEdge(out GEdge singleParent))
      {
        node = singleParent.ParentNode; //  we retrieve one here arbitrarily
      }
      else
      {
        break;
