...
public partial class MCGSManager
{
    ...
    //      if (BUILD_TREE_MODE)
    {
      using (new TimingBlock("build/use"))
      {
        ...
        graph.Validate();
          ...
          {
            using (new TimingBlock("Select/Backup"))
            {
              const int SELECTOR_ID = 0;
              for (int i = 0; i < 5; i++)
              {
                int numTargetVisits = 1;// i <= 2 ? 1 : 2;
                const bool ENABLE_STATE = false;
                RunBatchSelectAndBackup(graph, SELECTOR_ID, 3, ENABLE_STATE, 0, true);

                //            graph.Store.Validate(null, nodesInFlightExpectedZero:false, graphEnabled:Graph.GRAPH_ENABLED);
              }
            }
          }
#endif
                                                           ...
                                                           int nodeDepth, int maxDepth, int maxChildrenPerNode)
  {
    throw new NotImplementedException();
#if NOT
      if (numCreated++ > 100_000_000)
      {
        throw new Exception("too many nodes (temporary restriction to 100_000_000 in Graph.cs");
      }

      GameResult terminalStatus = pos.CalcTerminalStatus();
      if (terminalStatus == GameResult.Draw || terminalStatus == GameResult.Checkmate)
      {
        return;
      }

      Debug.Assert(!pos.CalcTerminalStatus().IsTerminal());
      ...
      int numMovesToUse = Math.Min(moves.NumMovesUsed, maxChildrenPerNode);

      // Cause the sequence of MoveInfoStruct to be allocated.
      GNodeEdgeHeaders headers = node.AllocatedEdgeHeaders(numMovesToUse);

      for (int i = 0; i < numMovesToUse; i++)
      {
        MGMove childMove = moves.MovesArray[i];
        MGPosition childPosMG = MGPosition.MGPosAfterMove(mgPos, childMove);
        Position childPos = childPosMG.ToPosition;
        EncodedMove encodedMove = ConverterMGMoveEncodedMove.MGChessMoveToEncodedMove(childMove);

        // Create dummy MoveInfo information (policy, action) because not yet known.
        headers.HeaderStructsSpan[i] = new GEdgeHeaderStruct(encodedMove, FP16.NaN, FP16.NaN, FP16.NaN);

        // Create a new node corresponding to this child and edge to attach to it.
        MGMoveList linkedNodeMoves;
        GNode child = graph.AddEdgeToNewOrExistingNode(node, i, in childPosMG, out linkedNodeMoves, out bool wasLinkedToExisting);

        if (!wasLinkedToExisting)
        {
          CreateAllUnvisitedChildrenRecursive(graph, child, in childPosMG, childPos, nodeDepth + 1, maxDepth, maxChildrenPerNode);
        }
      }
#endif
  }
  ...
  public void RunBatchSelectAndBackup(PositionWithHistory pos, int targetNumVisits, bool debugMode)
  {
    ...
    RunLoopUntilTreeSize(pos, SearchLimit.NodesPerMove(targetNumVisits));
