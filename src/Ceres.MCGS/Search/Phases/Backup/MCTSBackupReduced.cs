      ...
      foreach (MCGSPathVisitMember visitPair in path.PathVisitsLastBackedUpToRoot)
      {
        ...
        using (new NodeLockBlock(parentNode))
        {
          //          using (new NodeLockBlock(visitPathRef.ChildNode, true)) // Child might be null (if terminal) hence allow null lock
          {
            ...
            {
              if (rpoLambda > 0 && visitEdge.ChildNode.NumEdgesVisited > 1)
              {
            ...
            if (numVisitsAccepted > 0)
            {
              ...
              if (visitPathRef.IndexOfChildInParent >= parentNode.NumEdgesVisited)
              {
                parentNode.NodeRef.NumEdgesVisited = (byte)(visitPathRef.IndexOfChildInParent + 1);
              }
              ...
              {
                // Note that edges don't contain D information
                // therefore the best we can do is pass the new known D for the child.
                // However this seemingly doesn't fully capture updates the way Q/W currently does.
                // TODO: consider if this could be improved (without needing to widen GEdgeStruct).
                double newParentD = 0;// ************ TO BE FIXED ************
                strategy.BackupToNode(parentNode, numVisitsAccepted, newEdgeDeltaW, newParentD);
              }
            }
          }
