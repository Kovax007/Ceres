    ...
    internal void ApplyLeafNodeUpdates(MCGSPath path)
    {
      ...
      switch (path.TerminationReason)
      {
        case MCGSPathTerminationReason.PendingNeuralNetEval:
          ...
          Engine.Strategy.BackupToNode(leafNode, numToApply, path.TerminationInfo.V, overrideDrawP ?? path.TerminationInfo.DrawP);

          break;

        case MCGSPathTerminationReason.TranspositionCopyValues:
          Engine.Strategy.BackupToNode(leafNode, numToApply, leafNode.V, overrideDrawP ?? leafNode.DrawP);
          break;
