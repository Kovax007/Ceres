...
public struct FloatPairCompressedToByte
{
  ...
  private static int Encode(float value)
  {
    ...
    // Round to nearest representable code (0 … 14).
    int code = (int)MathF.Round(value * EncodeScale);
    if (code < 0)
    {
      code = 0;
    }
    else if (code > NumericMaxCode)
    {
      code = NumericMaxCode;
    }
  ...
  private static float Decode(int code)
  {
    return code == NaNCode ? float.NaN : code * DecodeStep;
  }
  ...
  public override bool Equals(object obj) => obj is FloatPairCompressedToByte other && Equals(other);

  public override int GetHashCode() => packedData;

  public static bool operator ==(FloatPairCompressedToByte left, FloatPairCompressedToByte right) => left.Equals(right);  

  public static bool operator !=(FloatPairCompressedToByte left, FloatPairCompressedToByte right) => !(left == right);  
