...
public struct SpinLockByte
{
  ...
  const bool CHECK_THREAD_REENTRANCY = MCGSParamsFixed.ENABLE_EXTENDED_RELEASE_ASSERTIONS;
  ...
  public void Acquire()
  {
    MCGSParamsFixed.Assert(state != VALUE_ILLEGAL, "AcquireFoundIllegal");
    ...
    if (CHECK_THREAD_REENTRANCY)
    {
      ...
      {
        MCGSParamsFixed.Assert(!IsKnownLockedByThisThread, "ThreadReacquire");
      }
    }
  ...
  // Maps thread IDs into the safe tracked range [2..253], avoiding 0, 1, 254, 255.

/// <summary>
/// Maps thread IDs into the safe tracked range[2..256].
/// </summary>
...
/// <returns></returns>
  ...
  private static bool IsTrackableThreadId(int threadID)
    => threadID >= VALUE_TRACKED_MIN && threadID <= VALUE_TRACKED_MAX;
  ...
  public void Release()
  {
    MCGSParamsFixed.Assert(IsLocked, "ReleaseFoundUnlocked");
    MCGSParamsFixed.Assert(state != VALUE_ILLEGAL, "ReleaseFoundIllegal");

    if (CHECK_THREAD_REENTRANCY)
    {
      MCGSParamsFixed.Assert(IsPossiblyLockedByThisThread, "ReleaseFromWrongThread");
    }
  ...
  public bool IsPossiblyLockedByThisThread
  {
    get
    {
      ...
      {
        return true; // we didn't record a trackable owner
      }
