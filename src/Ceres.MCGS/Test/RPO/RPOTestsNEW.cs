...
public static class RPOTestsNEW
{
  ...
  public static double CalcRPOQ(GNode node, double qWhenNoChildren, int numChildrenToConsider, double rpoLambdaP)
  {
    ...
    int numVisited = node.NumEdgesVisited;

    Span<(double Q, double P, double U)> actions = stackalloc (double Q, double P, double U)[numVisited];
    ...
    Span<double> posteriorP = stackalloc double[numVisited];
    Span<double> posteriorQ = stackalloc double[numVisited];
...
public static class ReverseKlPosteriorPolicy
{
  ...
  /// </summary>
  public static void ComputePosterior(
  ...
  private static void ApplyUncertaintyMode(
  ...
  private static bool TrySolveAlphaBisection(
      ReadOnlySpan<double> coeff,
